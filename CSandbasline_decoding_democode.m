...
load('ca1_tracesod2.mat') % Ca data

% % loading defeat pure neuron ID of sleep
load('ca1tracesod2_sleep_defeatneurons_pure.mat');

%% load ca data 

% % 3.Extracting defeat pure neurons of sleep

ca_raw_data = ca1_tracesod2(:,reactivated_defeatneurons_pure);

bin_frame_num    = 20;   % 20   -> 1s, 1s binning, For PCA-ICA, I reccomend to not change here, first.


%% %%  ca1 trace sod 2 data session information


data_cell{1+shift,1} = ('pre habituation');   data_cell{1+shift,2} = [1:3000] ;   
data_cell{2+shift,1} = ('trace habituation');    data_cell{2+shift,2} = [3001:23000] ;  
data_cell{3+shift,1} = ('awake rest');   data_cell{3+shift,2} = [23001:26000]; 
data_cell{4+shift,1} = ('post habituation sleep');   data_cell{4+shift,2} = [26001:47800] ; 

data_cell{5+shift,1} = ('conditioning day homecage');    data_cell{5+shift,2} = [47801:50800] ; 
data_cell{6+shift,1} = ('trace conditioning');    data_cell{6+shift,2} = [50801:70800] ; 
data_cell{7+shift,1} = ('awake rest');    data_cell{7+shift,2} = [70801:73800] ; 
data_cell{8+shift,1} = ('post trace conditioning sleep');    data_cell{8+shift,2} = [73801:101000] ; 

data_cell{9+shift,1} = ('trace recall homecage');    data_cell{9+shift,2} = [101001:104000] ; 
data_cell{10+shift,1}= ('trace recall');  data_cell{10+shift,2}= [104001:140000] ; 
data_cell{11+shift,1}= ('awake rest'); data_cell{11+shift,2}= [140001:143000] ;
data_cell{12+shift,1}= ('post trace recall sleep'); data_cell{12+shift,2}= [143001:173200] ;

data_cell{13+shift,1}= ('trace re-conditioning homecage'); data_cell{13+shift,2}= [173201:176200] ;
data_cell{14+shift,1}= ('trace re-conditioning'); data_cell{14+shift,2}= [176201:196200] ;

data_cell{15+shift,1}= ('homecage'); data_cell{15+shift,2}= [196201:199200] ;
data_cell{16+shift,1}= ('sod day 1 sensory session 1'); data_cell{16+shift,2}= [199201:204000] ;
data_cell{17+shift,1}= ('sod day 1 sensory session 2'); data_cell{17+shift,2}= [204001:208800] ;
data_cell{18+shift,1}= ('awake rest'); data_cell{18+shift,2}= [208801:211800] ;
data_cell{19+shift,1}= ('post sod day 1 sleep'); data_cell{19+shift,2}= [211801:244400] ;

data_cell{20+shift,1}= ('sod day 2 session 1'); data_cell{20+shift,2}= [244401:249200] ;
data_cell{21+shift,1}= ('sod day 2 session 2'); data_cell{21+shift,2}= [249201:254000] ;

data_cell{22+shift,1}= ('sod day 3 session 1'); data_cell{22+shift,2}= [254001:258800] ;
data_cell{23+shift,1}= ('sod day 3 session 2'); data_cell{23+shift,2}= [258801:263600] ;

data_cell{24+shift,1}= ('sod day 4 session 1'); data_cell{24+shift,2}= [263601:268400] ;
data_cell{25+shift,1}= ('sod day 4 session 2'); data_cell{25+shift,2}= [268401:273200] ;

data_cell{26+shift,1}= ('sod day 5 session 1'); data_cell{26+shift,2}= [273201:278000] ;
data_cell{27+shift,1}= ('sod day 5 session 2'); data_cell{27+shift,2}= [278001:282800] ;

data_cell{28+shift,1}= ('sod day 6 homecage'); data_cell{28+shift,2}= [282801:285800] ;
data_cell{29+shift,1}= ('sod day 6 sensory 1'); data_cell{29+shift,2}= [285801:290600] ;
data_cell{30+shift,1}= ('sod day 6 sensory 2'); data_cell{30+shift,2}= [290601:295400] ;
data_cell{31+shift,1}= ('sod day 6 awake rest'); data_cell{31+shift,2}= [295401:298400] ;
data_cell{32+shift,1}= ('sod day 6 sleep'); data_cell{32+shift,2}= [298401:345000] ;

data_cell{33+shift,1}= ('novel tonehomecage'); data_cell{33+shift,2}= [345001:348000] ;
data_cell{34+shift,1}= ('novel tone'); data_cell{34+shift,2}= [348001:393600] ;
data_cell{35+shift,1}= ('novel tone awake'); data_cell{35+shift,2}= [393601:396600] ;
data_cell{36+shift,1}= ('novel tone sleep'); data_cell{36+shift,2}= [396601:420600] ;

data_cell{37+shift,1}= ('pre trace recall homecage'); data_cell{37+shift,2}= [420601:423600] ;
data_cell{38+shift,1}= ('trace recall 2 '); data_cell{38+shift,2}= [423601:459600] ;
